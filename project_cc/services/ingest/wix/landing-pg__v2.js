 ...
 * 
 * PARENT: dropdown-options-container_-26
 * CHILD: dropdown-options-container_-18
 * VERSION 1.0.1
 */
...
$w.onReady(async () => {
  // Initial UI state
  $w('#dropdown-options-container_-18').options = [];
  $w('#dropdown-options-container_-18').disable();

  // Load + index categories (from cache if valid)
  categoryIndex = await loadCategoryIndex();

  // Populate parent dropdown
  $w('#dropdown-options-container_-26').options = categoryIndex.parentOptions;

  // Optional: If you want a placeholder at the top:
  // $w('#dropdown-options-container_-26').placeholder = "Select a category";
  // $w('#dropdown-options-container_-18').placeholder = "Select a sub-category";
});
...
export function parent_category_change(event) {
  ...
  $w('#dropdown-options-container_-18').options = subOptions.map(o => ({
    label: o.label,
    value: o.value
  }));

  $w('#dropdown-options-container_-18').enable();
}
